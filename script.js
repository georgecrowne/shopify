    ...
    if (!selected.some(row => row.includes(id))) {
      ...
      for (var i = 1; i < selected.length + 1; i++) {
        ...
        document.getElementById("nominated_" + i).addEventListener("mouseleave", function() {
          if (this.nomination_number <= selected.length) {
            document.getElementById("nominated_" + this.nomination_number).innerHTML = ""
            var text = document.createElement("P")
            text.id = "nominated_text_" + this.movieID
            text.innerHTML = selected[this.nomination_number - 1][1]
            text.classList.add("nominated_text")
            document.getElementById("nominated_" + this.nomination_number).appendChild(text)
          }
        })
      }
    }
